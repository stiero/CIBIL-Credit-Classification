...
median_actualpaymentamount <- account %>% select(customer_no, actualpaymentamount) %>% 
  group_by(customer_no) %>% summarise("median_actualpaymentamount" = 
                                        mean(actualpaymentamount, na.rm=TRUE))
...
data_new_test <- left_join(data_test, sum_paymenthistory_avg_dpd_0_29_bucket, by=c("customer_no"))

data_new_test <- left_join(data_new_test, last_365day_enquiries, by=c("customer_no"))
data_new_test$last_365day_enquiries[is.na(data_new_test$last_365day_enquiries)] <- 0

data_new_test <- left_join(data_new_test, last_90day_enquiries, by=c("customer_no"))
data_new_test$last_90day_enquiries[is.na(data_new_test$last_90day_enquiries)] <- 0


#data_new_test <- left_join(data_new_test, max_freq_enquiry, by=c("customer_no"))

data_new_test <- left_join(data_new_test, mean_diff_lastpayment_opened_dt, by=c("customer_no"))

data_new_test <- left_join(data_new_test, mean_diff_open_enquiry_dt, by=c("customer_no"))

data_new_test <- left_join(data_new_test, num_of_times_overdue, by=c("customer_no"))

data_new_test <- left_join(data_new_test, payment_history_mean_length, by=c("customer_no"))

data_new_test <- left_join(data_new_test, ratio_currbalance_creditlimit, by=c("customer_no"))

data_new_test <- left_join(data_new_test, sum_paymenthistory_avg_dpd_0_29_bucket, by=("customer_no"))

data_new_test <- left_join(data_new_test, temp, by=("customer_no"))

data_new_test <- left_join(data_new_test, utilisation_trend, by=c("customer_no"))

data_new_test <- left_join(data_new_test, diff_payment, by="customer_no")

data_new_test <- left_join(data_new_test, median_high_credit, by="customer_no")

data_new_test <- left_join(data_new_test, median_credit_limit, by="customer_no")

data_new_test <- left_join(data_new_test, median_cash_limit, by="customer_no")

data_new_test <- left_join(data_new_test, mean_rateofinterest, by="customer_no")

data_new_test <- left_join(data_new_test, median_actualpaymentamount, by="customer_no")

data_new_test <- left_join(data_new_test, median_enq_amt, by="customer_no")
